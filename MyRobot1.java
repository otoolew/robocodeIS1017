...
public class MyRobot1 extends AdvancedRobot{
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		setTurnGunRight(efficientRotation(getHeading() - getGunHeading() + e.getBearing()+(targetDeg*3)));
		// Fire Gun when Ready
	...
	public static void setProperties(String bullet, String step, String left, String right){
		//initializing Properties props
		Properties props = new Properties();
		// Try to load the file
		try {
			...
			props.load(new FileInputStream("config/MyRobot1.properties"));	 
			// set each Robot Property to the specified KEY. The Key is the in String Quotes and functions like a Hash
			props.setProperty("bulletenergy", bullet);
			props.setProperty("stepdistance", step);
			props.setProperty("leftrotationangle", left);
			props.setProperty("rightrotationangle", right);

			// The Catch Clause
		} catch (IOException e) {
			System.out.println("Failied to Set Props");
		}
